...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; font-family: var(--font-main); -webkit-tap-highlight-color: transparent; }
        body { background-color: var(--bg); color: var(--text); margin: 0; height: 100vh; overflow: hidden; display: flex; justify-content: center; }
        ...
        .meta .name { font-size: 0.75rem; color: #888; display: block; }
        .value { font-family: var(--font-nums); font-size: 1.25rem; font-weight: 600; color: var(--primary); }

        .fab-keyboard {
            position: absolute; bottom: 25px; left: 20px; width: 60px; height: 60px;
            background: var(--primary); border-radius: 50%; display: flex; justify-content: center; align-items: center;
            box-shadow: 0 8px 25px rgba(0, 210, 255, 0.3); z-index: 500; cursor: pointer;
        }

        .calc-sheet {
            position: absolute; bottom: -100%; left: 0; width: 100%; 
            background: rgba(10, 10, 12, 0.98); backdrop-filter: blur(25px);
            padding: 20px 20px 40px; border-radius: 30px 30px 0 0;
            transition: 0.4s cubic-bezier(0.1, 0.7, 0.1, 1); z-index: 1000;
            border-top: 1px solid rgba(255,255,255,0.1);
        }
        .calc-sheet.show { bottom: 0; }
        .calc-display {
            background: rgba(255,255,255,0.05); padding: 15px; border-radius: 12px;
            text-align: left; margin-bottom: 15px; min-height: 55px;
            font-family: var(--font-nums); font-size: 1.6rem; color: var(--primary); 
        }

        .keypad { display: grid; grid-template-columns: repeat(4, 1fr); gap: 8px; }
        .keypad button {
            height: 58px; border-radius: 14px; border: none; font-family: var(--font-nums);
            font-size: 1.4rem; font-weight: 600; color: white; background: #1c1c1e;
        }
...
<div id="loader">
    <div style="width: 40px; height: 40px; border: 3px solid #111; border-top-color: var(--primary); border-radius: 50%; animation: spin 1s linear infinite;"></div>
</div>
...
<div class="app-container">
    <div class="header">
        ...
        <div class="search-wrapper">
            <input type="text" id="search" placeholder="ابحث في العالم..." oninput="filterList()">
        </div>
    </div>
    <div class="currency-list" id="list"></div>
    <div class="fab-keyboard" onclick="toggleCalc(true)">
        <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2.5"><rect x="2" y="5" width="20" height="14" rx="2"/><path d="M7 15h.01M12 15h.01M17 15h.01M7 10h.01M12 10h.01M17 10h.01"/></svg>
    </div>
    <div class="calc-sheet" id="calcSheet">
        ...
        <div class="keypad">
            ...
            <button style="background:#333" onclick="calcAction('DEL')">⌫</button>
...
<script>
    ...
    // القائمة الكاملة والنهائية (160 دولة) - مفحوصة يدوياً
    const db = [
        ...
        { code: "VES", name: "فنزويلا", flag: "ve" }, { code: "ANG", name: "أنغولا", flag: "ao" }, { code: "BHD", name: "بنغلاديش", flag: "bd" },
        { code: "BND", name: "بروناي", flag: "bn" }, { code: "KHR", name: "كمبوديا", flag: "kh" }, { code: "CYP", name: "قبرص", flag: "cy" },
        { code: "IRR", name: "إيران", flag: "ir" }, { code: "KGS", name: "قيرغيزستان", flag: "kg" }, { code: "LAK", name: "لاوس", flag: "la" },
        { code: "MVR", name: "المالديف", flag: "mv" }, { code: "MNT", name: "منغوليا", flag: "mn" }, { code: "MMK", name: "ميانمار", flag: "mm" },
        { code: "NPR", name: "نيبال", flag: "np" }, { code: "KPW", name: "كوريا الشمالية", flag: "kp" }, { code: "LKR", name: "سريلانكا", flag: "lk" },
        { code: "TJS", name: "طاجيكستان", flag: "tj" }, { code: "TMT", name: "تركمانستان", flag: "tm" }, { code: "ETB", name: "إثيوبيا", flag: "et" },
        { code: "KES", name: "كينيا", flag: "ke" }, { code: "GHS", name: "غانا", flag: "gh" }, { code: "TZS", name: "تنزانيا", flag: "tz" },
        { code: "UGX", name: "أوغندا", flag: "ug" }, { code: "ZMW", name: "زامبيا", flag: "zm" }, { code: "ZWL", name: "زيمبابوي", flag: "zw" },
        { code: "ISK", name: "أيسلندا", flag: "is" }, { code: "RON", name: "رومانيا", flag: "ro" }, { code: "HUF", name: "المجر", flag: "hu" },
        { code: "CZK", name: "تشيكيا", flag: "cz" }, { code: "BGN", name: "بلغاريا", flag: "bg" }, { code: "HRK", name: "كرواتيا", flag: "hr" }
    ];

    async function detectLocation() {
        try {
            const loc = await fetch('https://ipapi.co/json/').then(r => r.json());
            if(loc.currency && loc.currency !== "ILS") base = loc.currency;
        } catch(e) {}
        // ترتيب: الموقع الحالي أولاً، الدولار ثانياً، ثم البقية
    ...
    function toggleCalc(show) {
        document.getElementById('calcSheet').classList.toggle('show', show);
        if(show) history.pushState({open:true}, "");
    }
    window.onpopstate = () => document.getElementById('calcSheet').classList.remove('show');

    async function setBase(code) {
        base = code; amount = "0"; expression = "0";
